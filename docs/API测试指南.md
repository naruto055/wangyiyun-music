# API 测试指南

**创建时间**: 2026-01-24
**最后更新**: 2026-01-30
**项目**: 网易云音乐后端系统

---

## 📋 测试前准备

### 1. 启动应用

**方式 1: IDE 运行**
- 在 IntelliJ IDEA 中找到 `WangyiyunMusicApplication.java`
- 右键 → Run 'WangyiyunMusicApplication'

**方式 2: Maven 命令**
```bash
mvn spring-boot:run
```

**方式 3: JAR 包运行**
```bash
mvn clean package
java -jar target/wangyiyun-music-0.0.1-SNAPSHOT.jar
```

**启动成功标志**: 控制台输出 "Started WangyiyunMusicApplication"
**访问地址**: http://localhost:8910

---

## 🎵 API 接口清单

### 1. 音乐列表查询

**接口**: `GET /api/music/list`
**功能**: 分页查询音乐，支持分类筛选、关键词搜索、排序

**请求参数**:
| 参数 | 类型 | 必填 | 说明 | 示例 |
|------|------|------|------|------|
| pageNum | Integer | 否 | 页码 | 1 |
| pageSize | Integer | 否 | 每页大小 | 10 |
| categoryId | Long | 否 | 分类ID | 1 |
| keyword | String | 否 | 关键词（歌曲名） | 晴天 |
| sortField | String | 否 | 排序字段(play_count/create_time) | play_count |
| sortOrder | String | 否 | 排序方式(asc/desc) | desc |

**测试用例**:

```bash
# 用例 1: 查询所有音乐（分页）
curl -X GET "http://localhost:8910/api/music/list?pageNum=1&pageSize=10"

# 用例 2: 按播放量降序排序
curl -X GET "http://localhost:8910/api/music/list?sortField=play_count&sortOrder=desc"

# 用例 3: 搜索关键词"晴天"
curl -X GET "http://localhost:8910/api/music/list?keyword=晴天"

# 用例 4: 查询分类ID=1的音乐
curl -X GET "http://localhost:8910/api/music/list?categoryId=1"
```

**预期响应**:
```json
{
  "code": 200,
  "message": "操作成功",
  "data": {
    "records": [
      {
        "id": 1,
        "title": "晴天",
        "duration": 269,
        "playCount": 15678,
        "favoriteCount": 3245,
        ...
      }
    ],
    "total": 6,
    "size": 10,
    "current": 1,
    "pages": 1
  }
}
```

---

### 2. 音乐详情查询

**接口**: `GET /api/music/{id}`
**功能**: 获取音乐详细信息，包含歌手、专辑、分类、标签等

**路径参数**:
| 参数 | 类型 | 说明 |
|------|------|------|
| id | Long | 音乐ID |

**测试用例**:

```bash
# 查询ID=1的音乐详情
curl -X GET "http://localhost:8910/api/music/1"
```

**预期响应**:
```json
{
  "code": 200,
  "message": "操作成功",
  "data": {
    "id": 1,
    "title": "晴天",
    "duration": 269,
    "fileUrl": "http://music.example.com/sunny_day.mp3",
    "coverUrl": "http://music.example.com/covers/sunny_day.jpg",
    "lyrics": "故事的小黄花...",
    "playCount": 15678,
    "favoriteCount": 3245,
    "releaseDate": "2003-07-31",
    "albumName": "叶惠美",
    "categoryName": "流行",
    "artists": [
      {
        "id": 1,
        "name": "周杰伦",
        "role": "singer"
      }
    ],
    "tags": ["华语", "抒情", "经典", "校园"]
  }
}
```

---

### 3. 记录播放

**接口**: `POST /api/play-record`
**功能**: 记录音乐播放并自动更新播放次数

**请求体**:
```json
{
  "musicId": 1,
  "playDuration": 180,
  "playSource": "web"
}
```

**测试用例**:

```bash
# 记录播放
curl -X POST "http://localhost:8910/api/play-record" \
  -H "Content-Type: application/json" \
  -d '{"musicId": 1, "playDuration": 180, "playSource": "web"}'
```

**预期响应**:
```json
{
  "code": 200,
  "message": "操作成功",
  "data": null
}
```

**验证**: 再次查询音乐详情，`playCount` 应该 +1

---

### 4. 查询播放历史

**接口**: `GET /api/play-record/history`
**功能**: 查询用户播放历史记录（分页）

**请求参数**:
| 参数 | 类型 | 必填 | 说明 | 默认值 |
|------|------|------|------|--------|
| pageNum | Integer | 否 | 页码 | 1 |
| pageSize | Integer | 否 | 每页大小 | 10 |

**测试用例**:

```bash
# 查询播放历史
curl -X GET "http://localhost:8910/api/play-record/history?pageNum=1&pageSize=10"
```

**预期响应**:
```json
{
  "code": 200,
  "message": "操作成功",
  "data": {
    "records": [
      {
        "id": 1,
        "musicId": 1,
        "userId": 0,
        "playDuration": 180,
        "playSource": "web",
        "createTime": "2026-01-24T15:30:00"
      }
    ],
    "total": 4,
    "size": 10,
    "current": 1
  }
}
```

---

### 5. 收藏音乐

**接口**: `POST /api/favorite/{musicId}`
**功能**: 收藏音乐并自动更新收藏次数

**路径参数**:
| 参数 | 类型 | 说明 |
|------|------|------|
| musicId | Long | 音乐ID |

**测试用例**:

```bash
# 收藏ID=1的音乐
curl -X POST "http://localhost:8910/api/favorite/1"
```

**预期响应**:
```json
{
  "code": 200,
  "message": "操作成功",
  "data": null
}
```

**验证**: 再次查询音乐详情，`favoriteCount` 应该 +1

**重复收藏测试**:
```bash
# 再次收藏同一首歌
curl -X POST "http://localhost:8910/api/favorite/1"
```

**预期响应**（错误）:
```json
{
  "code": 500,
  "message": "已收藏该音乐",
  "data": null
}
```

---

### 6. 查询收藏列表

**接口**: `GET /api/favorite/list`
**功能**: 查询用户收藏的音乐列表（分页）

**请求参数**:
| 参数 | 类型 | 必填 | 说明 | 默认值 |
|------|------|------|------|--------|
| pageNum | Integer | 否 | 页码 | 1 |
| pageSize | Integer | 否 | 每页大小 | 10 |

**测试用例**:

```bash
# 查询收藏列表
curl -X GET "http://localhost:8910/api/favorite/list?pageNum=1&pageSize=10"
```

**预期响应**:
```json
{
  "code": 200,
  "message": "操作成功",
  "data": {
    "records": [
      {
        "id": 1,
        "musicId": 1,
        "userId": 0,
        "isDeleted": 0,
        "createTime": "2026-01-24T15:30:00"
      }
    ],
    "total": 2,
    "size": 10,
    "current": 1
  }
}
```

---

### 7. 取消收藏

**接口**: `DELETE /api/favorite/{musicId}`
**功能**: 取消收藏（逻辑删除）并自动更新收藏次数

**路径参数**:
| 参数 | 类型 | 说明 |
|------|------|------|
| musicId | Long | 音乐ID |

**测试用例**:

```bash
# 取消收藏ID=1的音乐
curl -X DELETE "http://localhost:8910/api/favorite/1"
```

**预期响应**:
```json
{
  "code": 200,
  "message": "操作成功",
  "data": null
}
```

**验证**:
1. 再次查询收藏列表，该记录应该消失
2. 查询音乐详情，`favoriteCount` 应该 -1

---

### 8. 获取音频访问URL

**接口**: `GET /api/audio/{musicId}`
**功能**: 根据音乐ID获取音频文件访问URL，支持HTTP Range请求

**路径参数**:
| 参数 | 类型 | 说明 |
|------|------|------|
| musicId | Long | 音乐ID |

**测试用例**:

```bash
# 用例 1: 获取ID=1的音频URL
curl -X GET "http://localhost:8910/api/audio/1"
```

**预期响应**:
```json
{
  "code": 200,
  "message": "操作成功",
  "data": {
    "audioUrl": "http://localhost:8910/audio/jay/晴天.mp3",
    "musicId": 1,
    "title": "晴天",
    "fileName": "晴天.mp3",
    "duration": 269
  }
}
```

**错误测试**:
```bash
# 用例 2: 测试不存在的音乐ID
curl -X GET "http://localhost:8910/api/audio/999"
```

**预期响应**（错误）:
```json
{
  "code": 500,
  "message": "音乐不存在，ID: 999",
  "data": null
}
```

**前端集成测试**:
```html
<!-- 在浏览器中测试音频播放 -->
<audio src="http://localhost:8910/audio/jay/晴天.mp3" controls></audio>
```

> **说明**：
> - `audioUrl` 可直接用于 HTML5 `<audio>` 标签
> - 支持拖拽播放（HTTP Range请求）
> - 浏览器可以缓存音频文件

---

## 🧪 完整测试流程

### 流程 1: 音乐播放流程

```bash
# 1. 查询音乐列表
curl -X GET "http://localhost:8910/api/music/list"

# 2. 获取音乐详情（记录初始播放次数）
curl -X GET "http://localhost:8910/api/music/1"

# 3. 记录播放
curl -X POST "http://localhost:8910/api/play-record" \
  -H "Content-Type: application/json" \
  -d '{"musicId": 1, "playDuration": 269, "playSource": "web"}'

# 4. 验证播放次数增加
curl -X GET "http://localhost:8910/api/music/1"

# 5. 查询播放历史
curl -X GET "http://localhost:8910/api/play-record/history"
```

### 流程 2: 音乐收藏流程

```bash
# 1. 收藏音乐
curl -X POST "http://localhost:8910/api/favorite/1"

# 2. 验证收藏次数增加
curl -X GET "http://localhost:8910/api/music/1"

# 3. 查询收藏列表
curl -X GET "http://localhost:8910/api/favorite/list"

# 4. 取消收藏
curl -X DELETE "http://localhost:8910/api/favorite/1"

# 5. 验证收藏次数减少
curl -X GET "http://localhost:8910/api/music/1"
```

### 流程 3: 音频播放流程

```bash
# 1. 查询音乐列表，选择要播放的音乐
curl -X GET "http://localhost:8910/api/music/list"

# 2. 获取音频URL
curl -X GET "http://localhost:8910/api/audio/1"

# 3. 使用audioUrl在前端播放器中播放（支持拖拽）
# 前端代码示例：
# <audio src="http://localhost:8910/audio/jay/晴天.mp3" controls></audio>

# 4. 播放完成后，记录播放历史
curl -X POST "http://localhost:8910/api/play-record" \
  -H "Content-Type: application/json" \
  -d '{"musicId": 1, "playDuration": 269, "playSource": "web"}'

# 5. 验证播放次数增加
curl -X GET "http://localhost:8910/api/music/1"

# 6. 查询播放历史，确认记录已保存
curl -X GET "http://localhost:8910/api/play-record/history"
```

---

## ✅ 验证清单

完整测试后，确认以下功能正常：

- [ ] 音乐列表查询（分页正常）
- [ ] 音乐列表搜索（关键词生效）
- [ ] 音乐列表排序（按播放量/创建时间）
- [ ] 音乐详情查询（包含歌手、专辑、标签）
- [ ] 播放记录功能（播放次数自动+1）
- [ ] 播放历史查询（分页正常）
- [ ] 收藏功能（收藏次数自动+1）
- [ ] 收藏列表查询（分页正常）
- [ ] 取消收藏功能（收藏次数自动-1）
- [ ] 重复收藏校验（提示"已收藏该音乐"）
- [ ] 异常处理（音乐不存在、参数校验等）
- [ ] 音频URL获取（正常返回audioUrl）
- [ ] 音频URL访问（支持HTTP Range请求）
- [ ] 音频文件播放（前端可正常播放）
- [ ] 音频不存在错误处理（返回错误信息）

---

## 🐛 常见问题

### 问题 1: 启动失败 - 数据库连接错误

**错误信息**: `Cannot create connection to database server`

**解决方案**:
1. 确认 MySQL 已启动
2. 检查 `application.yaml` 中的数据库连接信息
3. 确认数据库 `wangyiyun-music` 已创建

### 问题 2: 接口返回 404

**错误信息**: `404 Not Found`

**解决方案**:
1. 确认 URL 路径正确（注意 `/api` 前缀）
2. 确认应用已成功启动
3. 检查控制台是否有错误日志

### 问题 3: 接口返回 500 - 空指针异常

**错误信息**: `NullPointerException`

**解决方案**:
1. 确认测试数据已导入（`docs/sql/test_data.sql`）
2. 检查数据库中是否有数据
3. 查看控制台详细错误堆栈

---

## 📊 测试数据说明

测试数据包含：
- **6 首歌曲**: 晴天、七里香、稻香、江南、红豆、演员
- **5 位歌手**: 周杰伦、林俊杰、陈奕迅、邓紫棋、薛之谦
- **5 张专辑**: Jay、范特西、叶惠美、八度空间、JJ陆
- **6 个分类**: 流行、摇滚、古典、民谣、电子、说唱
- **8 个标签**: 华语、抒情、快歌、经典、怀旧、校园、治愈、励志

**默认 userId**: 所有接口暂时使用 `userId=0`，后续集成用户系统后替换。

---

**测试完成后，请记录任何发现的问题或改进建议。**
