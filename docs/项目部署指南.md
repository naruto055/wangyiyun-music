# 项目部署指南

> 📦 推荐使用一体化部署方案，所有文件在一个目录，简单高效

---

## 🎯 部署方案概览

### 推荐方案：一体化部署 ⭐

**核心理念**：将应用、数据、工具统一放在一个目录下，无需配置环境变量。

```
/opt/service/wangyiyun-music/          ← 部署根目录
├── wangyiyun-music-0.0.1-SNAPSHOT.jar ← 应用程序
├── music-data/                         ← 数据目录
│   ├── temp/                          ← 临时音频文件
│   └── audio/                         ← 永久音频文件
├── tools/                              ← 工具目录
│   └── yt-dlp                         ← 视频解析工具
└── logs/                               ← 日志目录
    └── application.log
```

**优势**：
- ✅ **零配置**：无需设置环境变量，创建目录即可
- ✅ **易理解**：所有文件在一起，一目了然
- ✅ **易迁移**：整体打包即可迁移到其他服务器
- ✅ **易备份**：一个命令备份所有数据

---

## 🚀 快速开始（5分钟）

### 方式 1：自动化部署（推荐）

```bash
# 1. 上传脚本到服务器
scp scripts/deploy-simple.sh user@server:/tmp/

# 2. 执行一键部署
ssh user@server
sudo bash /tmp/deploy-simple.sh
```

**脚本会自动完成**：
- ✅ 创建目录结构
- ✅ 部署应用 JAR
- ✅ 安装 yt-dlp 工具
- ✅ 配置 systemd 服务
- ✅ 设置文件权限
- ✅ 启动服务并验证

**完成后访问**：
- Swagger UI: http://服务器IP:8910/swagger-ui/index.html
- API 文档: http://服务器IP:8910/v3/api-docs

---

### 方式 2：手动部署（3步完成）

#### 步骤 1：创建目录结构

```bash
# 创建部署根目录（路径可自定义）
sudo mkdir -p /opt/service/wangyiyun-music

# 切换到部署目录
cd /opt/service/wangyiyun-music

# 创建子目录
sudo mkdir -p music-data/temp      # 临时音频文件
sudo mkdir -p music-data/audio     # 永久音频文件
sudo mkdir -p tools                 # 工具目录
sudo mkdir -p logs                  # 日志目录
```

#### 步骤 2：部署文件

```bash
# 2.1 复制应用 JAR 包
sudo cp /path/to/wangyiyun-music-0.0.1-SNAPSHOT.jar /opt/service/wangyiyun-music/

# 2.2 下载 yt-dlp 工具
sudo wget https://github.com/yt-dlp/yt-dlp/releases/latest/download/yt-dlp \
  -O /opt/service/wangyiyun-music/tools/yt-dlp

# 2.3 设置执行权限
sudo chmod +x /opt/service/wangyiyun-music/tools/yt-dlp

# 2.4 验证工具安装
/opt/service/wangyiyun-music/tools/yt-dlp --version
```

#### 步骤 3：启动应用

**方式 A：直接启动（测试环境）**

```bash
cd /opt/service/wangyiyun-music

# 前台运行（查看日志）
java -jar wangyiyun-music-0.0.1-SNAPSHOT.jar \
  --spring.profiles.active=prod

# 后台运行
nohup java -jar wangyiyun-music-0.0.1-SNAPSHOT.jar \
  --spring.profiles.active=prod > logs/app.log 2>&1 &
```

**方式 B：systemd 服务（生产环境推荐）**

创建服务文件 `/etc/systemd/system/wangyiyun-music.service`：

```ini
[Unit]
Description=WangYiYun Music Service
After=network.target mysql.service

[Service]
Type=simple
User=root
WorkingDirectory=/opt/service/wangyiyun-music

# JVM 参数配置
Environment="JAVA_OPTS=-Xms512m -Xmx2g -XX:+UseG1GC"

# 启动命令
ExecStart=/usr/bin/java $JAVA_OPTS \
  -jar /opt/service/wangyiyun-music/wangyiyun-music-0.0.1-SNAPSHOT.jar \
  --spring.profiles.active=prod

# 重启策略
Restart=on-failure
RestartSec=10

# 日志配置
StandardOutput=journal
StandardError=journal
SyslogIdentifier=wangyiyun-music

[Install]
WantedBy=multi-user.target
```

**启动服务**：

```bash
# 重载 systemd 配置
sudo systemctl daemon-reload

# 启动服务
sudo systemctl start wangyiyun-music

# 查看状态
sudo systemctl status wangyiyun-music

# 设置开机自启
sudo systemctl enable wangyiyun-music

# 查看实时日志
sudo journalctl -u wangyiyun-music -f
```

---

## ✅ 部署验证

### 1. 检查服务状态

```bash
# 查看服务状态
sudo systemctl status wangyiyun-music

# 查看端口监听
netstat -tuln | grep 8910
# 或
ss -tuln | grep 8910
```

### 2. 验证 API 访问

```bash
# 方式 1：检查 Swagger UI
curl -I http://localhost:8910/swagger-ui/index.html

# 方式 2：测试 API 接口
curl http://localhost:8910/api/music/list

# 方式 3：浏览器访问
# http://服务器IP:8910/swagger-ui/index.html
```

### 3. 验证目录结构

```bash
# 查看目录结构
tree -L 2 /opt/service/wangyiyun-music

# 预期输出：
# /opt/service/wangyiyun-music/
# ├── wangyiyun-music-0.0.1-SNAPSHOT.jar
# ├── music-data/
# │   ├── temp/
# │   └── audio/
# ├── tools/
# │   └── yt-dlp
# └── logs/
```

### 4. 验证日志输出

```bash
# 查看应用日志
tail -f /opt/service/wangyiyun-music/logs/application.log

# 查看 systemd 日志
sudo journalctl -u wangyiyun-music -n 50
```

---

## 🔧 配置说明

### 自动路径解析

应用会**自动识别**相对于部署目录的路径，无需手动配置：

| 配置项 | 自动解析值 | 实际路径示例 |
|--------|-----------|-------------|
| 音频文件存储 | `${user.dir}/music-data/audio/` | `/opt/service/wangyiyun-music/music-data/audio/` |
| 临时文件存储 | `${user.dir}/music-data/temp/` | `/opt/service/wangyiyun-music/music-data/temp/` |
| yt-dlp 工具 | `${user.dir}/tools/yt-dlp` | `/opt/service/wangyiyun-music/tools/yt-dlp` |

**关键**：
- ✅ `${user.dir}` 是 Java 的工作目录（通常是 JAR 包所在目录）
- ✅ systemd 服务中使用 `WorkingDirectory` 指定工作目录
- ✅ 手动启动时确保先 `cd` 到部署目录

### 部署目录自定义

可以部署在**任意位置**，只需确保目录结构一致：

```bash
# 示例 1：/opt/service/
/opt/service/wangyiyun-music/

# 示例 2：/home/user/
/home/user/wangyiyun-music/

# 示例 3：/usr/local/
/usr/local/wangyiyun-music/
```

**要点**：启动时确保工作目录正确

```bash
# systemd 服务
WorkingDirectory=/your/custom/path/

# 手动启动
cd /your/custom/path/
java -jar wangyiyun-music.jar
```

---

## 📊 运维管理

### 服务管理

```bash
# 启动服务
sudo systemctl start wangyiyun-music

# 停止服务
sudo systemctl stop wangyiyun-music

# 重启服务
sudo systemctl restart wangyiyun-music

# 查看状态
sudo systemctl status wangyiyun-music

# 开机自启
sudo systemctl enable wangyiyun-music

# 禁用自启
sudo systemctl disable wangyiyun-music
```

### 日志管理

```bash
# 查看实时日志
sudo journalctl -u wangyiyun-music -f

# 查看最近 100 行日志
sudo journalctl -u wangyiyun-music -n 100

# 查看今天的日志
sudo journalctl -u wangyiyun-music --since today

# 查看指定时间段日志
sudo journalctl -u wangyiyun-music --since "2026-01-31 10:00:00" --until "2026-01-31 12:00:00"
```

### 备份与恢复

**备份**（超简单）：

```bash
# 方式 1：整体备份
tar -czf wangyiyun-music-backup-$(date +%Y%m%d).tar.gz \
  /opt/service/wangyiyun-music

# 方式 2：仅备份数据
tar -czf music-data-backup-$(date +%Y%m%d).tar.gz \
  /opt/service/wangyiyun-music/music-data

# 方式 3：增量备份（rsync）
rsync -avz --delete \
  /opt/service/wangyiyun-music/ \
  /backup/wangyiyun-music-$(date +%Y%m%d)/
```

**恢复**：

```bash
# 停止服务
sudo systemctl stop wangyiyun-music

# 解压备份
tar -xzf wangyiyun-music-backup-20260131.tar.gz -C /

# 启动服务
sudo systemctl start wangyiyun-music
```

### 迁移到新服务器

```bash
# 旧服务器：打包
ssh old-server
tar -czf wangyiyun-music.tar.gz /opt/service/wangyiyun-music
scp wangyiyun-music.tar.gz new-server:/tmp/

# 新服务器：解压
ssh new-server
sudo tar -xzf /tmp/wangyiyun-music.tar.gz -C /opt/service/

# 新服务器：安装 Java（如未安装）
sudo apt install openjdk-17-jdk  # Debian/Ubuntu
# 或
sudo yum install java-17-openjdk  # CentOS/RHEL

# 新服务器：配置 systemd 并启动
# （参考上文"步骤 3：启动应用"）
```

---

## 🔒 安全建议

### 1. 使用专用用户（推荐）

```bash
# 创建应用用户（不允许登录）
sudo useradd -r -s /bin/false appuser

# 设置目录所有权
sudo chown -R appuser:appuser /opt/service/wangyiyun-music

# 修改 systemd 服务文件
# User=appuser
```

### 2. 限制目录权限

```bash
# 应用根目录：仅所有者可读写
sudo chmod 750 /opt/service/wangyiyun-music

# 数据目录：所有者和组可读写
sudo chmod 770 /opt/service/wangyiyun-music/music-data

# JAR 文件：仅所有者可读
sudo chmod 640 /opt/service/wangyiyun-music/*.jar
```

### 3. 防火墙配置

```bash
# 方式 1：ufw（Ubuntu/Debian）
sudo ufw allow from 192.168.1.0/24 to any port 8910

# 方式 2：firewalld（CentOS/RHEL）
sudo firewall-cmd --permanent --add-rich-rule='rule family="ipv4" source address="192.168.1.0/24" port protocol="tcp" port="8910" accept'
sudo firewall-cmd --reload

# 方式 3：iptables
sudo iptables -A INPUT -p tcp -s 192.168.1.0/24 --dport 8910 -j ACCEPT
sudo iptables -A INPUT -p tcp --dport 8910 -j DROP
```

---

## 🐳 Docker 部署（可选）

### Dockerfile

```dockerfile
FROM openjdk:17-jdk-slim

# 设置工作目录
WORKDIR /app

# 安装 yt-dlp
RUN apt-get update && \
    apt-get install -y wget python3 && \
    wget https://github.com/yt-dlp/yt-dlp/releases/latest/download/yt-dlp -O /app/tools/yt-dlp && \
    chmod +x /app/tools/yt-dlp && \
    apt-get clean && rm -rf /var/lib/apt/lists/*

# 复制应用 JAR
COPY target/wangyiyun-music-*.jar /app/app.jar

# 创建数据目录
RUN mkdir -p /app/music-data/temp /app/music-data/audio /app/logs

# 暴露端口
EXPOSE 8910

# 启动命令
ENTRYPOINT ["java", "-jar", "/app/app.jar", "--spring.profiles.active=prod"]
```

### docker-compose.yml

```yaml
version: '3.8'

services:
  wangyiyun-music:
    build: ..
    container_name: wangyiyun-music
    ports:
      - "8910:8910"
    volumes:
      # 持久化数据目录
      - ./data/temp:/app/music-data/temp
      - ./data/audio:/app/music-data/audio
      - ./logs:/app/logs
    environment:
      # JVM 参数
      JAVA_OPTS: "-Xms512m -Xmx2g"
    restart: unless-stopped
```

**启动容器**：

```bash
# 构建并启动
docker-compose up -d

# 查看日志
docker-compose logs -f wangyiyun-music

# 停止服务
docker-compose down
```

---

## ❓ 常见问题

### Q1: 应用启动后找不到数据目录？

**原因**：工作目录设置不正确。

**解决方案**：

```bash
# systemd 服务：确保设置 WorkingDirectory
WorkingDirectory=/opt/service/wangyiyun-music

# 手动启动：必须先切换到部署目录
cd /opt/service/wangyiyun-music
java -jar wangyiyun-music.jar
```

---

### Q2: 提示 yt-dlp 未找到？

**原因**：yt-dlp 未安装或路径不正确。

**解决方案**：

```bash
# 验证 yt-dlp 是否存在
ls -lh /opt/service/wangyiyun-music/tools/yt-dlp

# 重新下载
wget https://github.com/yt-dlp/yt-dlp/releases/latest/download/yt-dlp \
  -O /opt/service/wangyiyun-music/tools/yt-dlp
chmod +x /opt/service/wangyiyun-music/tools/yt-dlp

# 测试
/opt/service/wangyiyun-music/tools/yt-dlp --version
```

---

### Q3: 端口 8910 被占用？

**解决方案**：

```bash
# 方式 1：修改应用端口
# 启动时指定端口
java -jar wangyiyun-music.jar --server.port=8920

# 方式 2：停止占用端口的进程
# 查找占用进程
sudo lsof -i :8910
# 或
sudo netstat -tlnp | grep 8910

# 停止进程
sudo kill -9 <PID>
```

---

### Q4: 如何更新应用版本？

```bash
# 1. 停止服务
sudo systemctl stop wangyiyun-music

# 2. 备份旧版本
cd /opt/service/wangyiyun-music
sudo mv wangyiyun-music-0.0.1-SNAPSHOT.jar \
  wangyiyun-music-0.0.1-SNAPSHOT.jar.backup-$(date +%Y%m%d)

# 3. 复制新版本
sudo cp /path/to/new/wangyiyun-music-0.0.2-SNAPSHOT.jar \
  /opt/service/wangyiyun-music/

# 4. 更新 systemd 服务文件中的 JAR 文件名（如有变化）
sudo vim /etc/systemd/system/wangyiyun-music.service

# 5. 重载配置并启动
sudo systemctl daemon-reload
sudo systemctl start wangyiyun-music

# 6. 验证
sudo systemctl status wangyiyun-music
```

---

### Q5: 如何查看应用日志？

```bash
# 方式 1：systemd 日志
sudo journalctl -u wangyiyun-music -f

# 方式 2：应用日志文件（如已配置）
tail -f /opt/service/wangyiyun-music/logs/application.log

# 方式 3：查看最近 100 行
sudo journalctl -u wangyiyun-music -n 100

# 方式 4：查看错误日志
sudo journalctl -u wangyiyun-music -p err
```

---

### Q6: 数据量很大，如何使用独立磁盘？

**方式 1：软链接（推荐）**

```bash
# 1. 在大容量磁盘创建数据目录
sudo mkdir -p /data/music-storage/temp
sudo mkdir -p /data/music-storage/audio

# 2. 删除原有目录，创建软链接
cd /opt/service/wangyiyun-music
sudo rm -rf music-data
sudo ln -s /data/music-storage music-data

# 3. 启动应用（无需修改配置）
sudo systemctl start wangyiyun-music
```

**方式 2：环境变量**

```bash
# 修改 systemd 服务文件
sudo vim /etc/systemd/system/wangyiyun-music.service

# 添加环境变量
Environment="MUSIC_TEMP_PATH=/data/music-storage/temp/"
Environment="MUSIC_AUDIO_PATH=file:/data/music-storage/audio/"

# 重载并重启
sudo systemctl daemon-reload
sudo systemctl restart wangyiyun-music
```

---

## 🔍 故障排查

### 应用无法启动

```bash
# 1. 查看详细日志
sudo journalctl -u wangyiyun-music -n 100 --no-pager

# 2. 检查 Java 版本
java -version  # 需要 17+

# 3. 检查工作目录
cat /etc/systemd/system/wangyiyun-music.service | grep WorkingDirectory

# 4. 检查端口占用
sudo netstat -tlnp | grep 8910

# 5. 手动启动查看错误
cd /opt/service/wangyiyun-music
java -jar wangyiyun-music.jar --spring.profiles.active=prod
```

### 数据库连接失败

```bash
# 1. 检查数据库连接
mysql -h localhost -u root -p

# 2. 查看配置文件中的数据库配置
# 如使用外部配置文件，检查配置是否正确

# 3. 检查防火墙
sudo ufw status
```

### 性能问题

```bash
# 1. 调整 JVM 参数
# 编辑 systemd 服务文件
Environment="JAVA_OPTS=-Xms1g -Xmx4g -XX:+UseG1GC -XX:MaxGCPauseMillis=200"

# 2. 查看系统资源
top
free -h
df -h

# 3. 查看应用线程
jps  # 获取 PID
jstack <PID>
```

---

## 📚 高级配置

### 环境变量覆盖（可选）

如需自定义路径，可使用环境变量：

```bash
# 在 systemd 服务文件中添加
Environment="MUSIC_TEMP_PATH=/custom/path/temp/"
Environment="MUSIC_AUDIO_PATH=file:/custom/path/audio/"

# 或在启动脚本中设置
export MUSIC_TEMP_PATH=/custom/path/temp/
export MUSIC_AUDIO_PATH=file:/custom/path/audio/
java -jar wangyiyun-music.jar
```

**可用的环境变量**：

| 环境变量 | 说明 | 默认值 |
|---------|------|--------|
| `MUSIC_TEMP_PATH` | 临时文件路径 | `${user.dir}/music-data/temp/` |
| `MUSIC_AUDIO_PATH` | 音频文件路径 | `file:${user.dir}/music-data/audio/` |
| `SERVER_PORT` | 服务端口 | `8910` |

---

## 📖 相关文档

- 📋 [项目 AI 上下文](CLAUDE.md) - 项目整体说明
- 📖 [详细部署文档](docs/deployment-simple.md) - 详细部署步骤
- 🚀 [快速参考手册](docs/quick-reference.md) - 命令速查
- 📚 [部署文档导航](docs/README.md) - 文档索引
- 🔧 [自动化脚本](scripts/deploy-simple.sh) - 一键部署脚本
- 📖 [API 文档](docs/API接口文档.md) - API 接口说明
- 🎥 [视频解析功能](docs/视频解析功能说明.md) - 视频解析功能

---

## 🆘 获取帮助

遇到问题？按以下顺序查找答案：

1. **查看本文档"常见问题"章节**
2. **查看应用日志**：`sudo journalctl -u wangyiyun-music -n 100`
3. **检查目录结构**：`tree -L 2 /opt/service/wangyiyun-music`
4. **验证配置**：确认工作目录、端口、权限设置
5. **提交 Issue**：提供详细的错误日志和环境信息

---

## 📝 总结

### 一体化部署的核心优势

| 优势 | 说明 |
|------|------|
| **极简部署** | 3 步完成，无需配置环境变量 |
| **易于理解** | 所有文件在一个目录，目录结构清晰 |
| **便于迁移** | 整体打包即可迁移，无需重新配置 |
| **便于备份** | 一个命令备份所有数据 |
| **降低门槛** | 新手也能快速上手 |

### 推荐使用场景

- ✅ 传统服务器部署
- ✅ 中小规模应用（数据量 < 100GB）
- ✅ 快速原型验证
- ✅ 团队开发测试

### 何时考虑其他方案

- 数据量超大（TB 级），需要独立磁盘 → 使用软链接或环境变量
- 容器化部署 → 使用 Docker
- 云原生环境 → 使用 Kubernetes

---

**文档版本**: 1.0
**更新时间**: 2026-01-31
**维护者**: naruto
**反馈建议**: 欢迎提交 Issue 或 Pull Request
